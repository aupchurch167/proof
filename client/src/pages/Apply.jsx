...
export default function Apply() {
  ...
  if (notFound) return <div className="p-12 text-center text-muted">Organization not found.</div>;
  ...
  if (submitted) {
    return (
      <div className="max-w-xl mx-auto p-6 text-center">
        ...
        <p className="text-xs text-faint mt-8">Powered by Proof — proofcoi.com</p>
      </div>
    );
  }

  const labelClass = 'block text-sm font-medium text-ink-2 mb-1';
  const inputClass = 'w-full px-3 py-2.5 border rounded-control text-base sm:text-sm';
  const fileClass = 'w-full text-sm file:mr-3 file:py-2 file:px-3 file:rounded-control file:border file:border-line-strong file:bg-white file:text-sm file:cursor-pointer hover:file:bg-card-alt';

  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      <header className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold">{org.name}</h1>
        <p className="text-lg text-muted mt-1">Subcontractor Application Form</p>
      </header>

      {error && <div className="bg-bad-bg text-bad-text px-4 py-3 rounded-control mb-4 text-sm">{error}</div>}

      <form onSubmit={handleSubmit} className="space-y-4 bg-white border rounded-card p-4 sm:p-6">
        <div>
        ...
        <button type="submit" disabled={submitting}
          className="w-full bg-amber text-white px-4 py-3 rounded-control hover:bg-amber-hover disabled:opacity-50 text-sm font-medium">
          {submitting ? 'Submitting…' : 'Submit Application'}
        </button>
      </form>

      <p className="text-center text-xs text-faint mt-8">Powered by Proof — proofcoi.com</p>
    </div>
  );
}
